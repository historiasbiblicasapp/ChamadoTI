-- ============================================
-- Setup completo do banco para projetos que já tem profiles
...
-- FK de profiles agora, depois que departments existir
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Funcao auxiliar para RLS sem recursao
...
-- 9) Atualizar status antigos para resolvido se existirem
UPDATE tickets
SET status = 'resolved',
    updated_at = NOW()
WHERE status = 'closed';
