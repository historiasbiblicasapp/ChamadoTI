-- Fix 1: SECURITY DEFINER function to check role without RLS recursion
...
-- Fix 2: Drop and recreate the recursive profiles policy
DROP POLICY IF EXISTS "Admins can manage all profiles" ON profiles;
CREATE POLICY "Admins can manage all profiles" ON profiles FOR ALL
  USING (public.get_user_role(auth.uid()) = 'admin');

-- Fix 3: Replace all other admin-check policies to use the safe function
-- Departments
...
-- Fix 4: Create the get_tickets_by_month RPC function
